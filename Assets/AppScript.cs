...
public class AppScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // connect
        if(devices.Length > 0){
            _device = devices[0].OpenDevice();
            

            _depthVideoStream = _device.CreateVideoStream(Device.SensorType.Depth);

            var res = _depthVideoStream.VideoMode.Resolution;
            Debug.Log($"depth resolution: {res.Width}x{res.Height}");

            _depthMeshRenderer.Init(res.Width, res.Height);
    ...
    void OnDeviceConnectionStateChanged(DeviceInfo device){
      //  Debug.Log($"OnDeviceConnectionStateChanged: {device.Name}");
    }  
